...
export default function ServiceCard({
  ...
  const cardContent = (
    <Card
      className={cn(
        "group relative h-full overflow-hidden border-0 bg-white/80 backdrop-blur-sm shadow-xl transition-all duration-700 hover:-translate-y-4 hover:shadow-2xl",
        className,
      )}
    ...
    >
      ...
      <div className="absolute inset-0 rounded-2xl p-[1px] opacity-0 transition-opacity duration-500 group-hover:opacity-100">
        <div className={`absolute inset-0 rounded-2xl bg-gradient-to-r ${gradient}`} />
        <div className="absolute inset-[1px] rounded-2xl bg-white" />
      </div>

      {/* Background gradient overlay */}
      <div
        className={`absolute inset-0 rounded-2xl bg-gradient-to-br ${gradient} opacity-0 transition-opacity duration-500 group-hover:opacity-5`}
      />
      ...
      <CardHeader className="relative z-10 pb-4">
        <motion.div
          className={cn(
            ...
            iconClassName || `bg-gradient-to-br ${gradient}`,
          )}
      ...
      <CardContent className="relative z-10 pt-0">
        ...
        <div className="flex items-center justify-between">
          ...
          <motion.div
            className={`w-10 h-10 rounded-full bg-gradient-to-r ${gradient} flex items-center justify-center shadow-lg opacity-0 group-hover:opacity-100`}
      ...
      {/* Decorative elements */}
      <div
        className={`absolute -right-8 -top-8 h-24 w-24 rounded-full bg-gradient-to-br ${gradient} opacity-10 transition-opacity duration-500 group-hover:opacity-20`}
      />
      <div
        className={`absolute -left-4 -bottom-4 h-16 w-16 rounded-full bg-gradient-to-br ${gradient} opacity-5 transition-opacity duration-500 group-hover:opacity-15`}
      />
    </Card>
  )
